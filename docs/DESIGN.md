...
On Unix, renaming the sibling file publishes or replaces the final path atomically, after which Rivet opens the containing topic directory and calls `sync_all()` to request durability for the renamed directory entry. The implementation relies on `std::fs::rename` to replace an existing final file; Rust documents the platform-specific Unix and Windows behavior of that operation. On Windows, the temporary file contents are still flushed and synced before publication, but stable Rust does not provide the directory-handle behavior used here to sync the renamed directory entry. Windows junction/alias rejection has not been verified in a Windows environment, and power-loss durability of the renamed directory entry remains an explicit platform limitation; neither may be described as a tested crash-durable guarantee.
