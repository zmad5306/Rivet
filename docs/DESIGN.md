...
On Unix, renaming the sibling file publishes or replaces the final path atomically, after which Rivet opens the containing topic directory and calls `sync_all()` to request durability for the renamed directory entry. The implementation relies on `std::fs::rename` to replace an existing final file; Rust documents the platform-specific Unix and Windows behavior of that operation. On Windows, the temporary file contents are still flushed and synced before publication, but stable Rust does not provide the directory-handle behavior used here to sync the renamed directory entry. Windows junction rejection is verified by a Windows-only test that confirms a junction at the consumer-group directory is rejected without being followed or modifying its target. Power-loss durability of the renamed directory entry remains an explicit Windows platform limitation and may not be described as a tested crash-durable guarantee.
